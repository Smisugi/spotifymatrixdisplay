...
A Raspberry Pi Zero W RGB LED matrix display that shows the current Spotify track's album art via Last.fm scrobbling. No Spotify API setup required — just a free Last.fm account connected to Spotify.
...
│   └── shutdown_button.py    # Optional GPIO shutdown button listener
├── config/
...
- Set hostname, username, password, WiFi and enable SSH in settings
...
## Pre-Configuring for a Gift
If setting up on behalf of someone else, skip the portal by:
1. Creating their Last.fm account at last.fm/join
2. Connecting Spotify at last.fm/settings/applications
3. Manually creating settings.json with their details:
```bash
cat > ~/matrixdisplay/config/settings.json << 'EOF'
{
    "lastfm_username": "their_lastfm_username",
    "wifi_ssid": "their_wifi_ssid"
}
EOF
```
4. Pre-saving their WiFi:
```bash
sudo nmcli device wifi connect "SSID" password "PASSWORD"
```

## Gifting Checklist
- [ ] Delete settings.json before gifting for fresh portal experience
- [ ] Delete saved WiFi: `sudo nmcli connection delete preconfigured`
- [ ] Test full portal flow works
- [ ] Test display shows album art
- [ ] Include recipient instructions below
...
5. Play something on Spotify and enjoy!
